...
export default {
  async fetchSummaries({ commit }) {
    ...
    const forRents = data.find((i) => i.listing_purpose === ListingPurpose.RENT)
    commit('SET_RENT', forRents || { latests: [], counts: {} })

    const forSales = data.find((i) => i.listing_purpose === ListingPurpose.SALE)
    commit('SET_SALE', forSales || { latests: [], counts: {} })

    const forRentsOrSales = data.find(
      (i) => i.listing_purpose === ListingPurpose.RENT_OR_SALE
    )
    commit('SET_RENT_OR_SALE', forRentsOrSales || { latests: [], counts: {} })
  },
