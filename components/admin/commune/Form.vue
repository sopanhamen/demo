...
const { mapFields } = createHelpers({
  getterType: 'commune/getField',
  mutationType: 'commune/updateField'
})
...
export default {
  ...
  validations() {
    return {
      nameKhmer: { required, minLength: minLength(2) },
      nameEnglish: { required, minLength: minLength(2) },
  ...
  computed: {
    ...mapFields({
      ...
      code: 'form.code',
      province: 'form.district.province_id',
      country: 'form.district.province.country_id',
    ...
    provinces() {
      return this.getProvinces([this.country])
    },
    districts() {
      return this.getDistricts([this.province])
    }
  }
}
</script>
